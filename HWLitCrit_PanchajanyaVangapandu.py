...
'''
PROBLEM 0. Panchajanya Vangapandu, HW LitCrit
'''
...
def litCricFriend(wordList, text):
    ...
    # One line of code.
    dashText = lowerCaseText.replace("-", " ")
    ...
    for element in splitText:
        newWordList.append(element.strip(string.punctuation))
    ...
    for element in wordList:
        lowerWord = element.lower()
        accumulator += newWordList.count(lowerWord)
    return accumulator
    ...
    ratio = accumulator / int(len(newWordList))
    return ratio
...
bellsFreqAAn = print(litCricFriend(['a','an'], theBells))
cantoFreqAAn = print(litCricFriend(['a','an'], cantoXII))
bellsFreqThe = print(litCricFriend(['the'], theBells))
cantoFreqThe = print(litCricFriend(['the'], cantoXII))
...
Poe uses the words "a" and "an" 21 times while
Neruda uses it 3 times.
Poe uses the words "the" 82 times while
Neruda uses it 17 times.
'''
